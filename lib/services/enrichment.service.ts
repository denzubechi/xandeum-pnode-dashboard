 ...
 */
export async function enrichPNodesWithGeo(nodes: PNode[], concurrency = 10): Promise<PNode[]> {
  const enrichedNodes: PNode[] = []

  // Process in batches to avoid overwhelming the geo API
  for (let i = 0; i < nodes.length; i += concurrency) {
    const batch = nodes.slice(i, i + concurrency)
    const enrichedBatch = await Promise.all(batch.map((node) => enrichPNodeWithGeo(node)))
